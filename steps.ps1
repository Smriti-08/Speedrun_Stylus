...
function Find-PackagesNextjs {
  $currentDir = Get-Location
  $searchDirs = @($currentDir)
  
  # Search in current directory and subdirectories
  foreach ($dir in $searchDirs) {
    $packagesNextjsPath = Join-Path $dir "packages/nextjs"
    if (Test-Path $packagesNextjsPath) {
      return $packagesNextjsPath
    }
    
    # Also search in subdirectories
    $subDirs = Get-ChildItem -Path $dir -Directory -ErrorAction SilentlyContinue
    foreach ($subDir in $subDirs) {
      $subPackagesPath = Join-Path $subDir.FullName "packages/nextjs"
      if (Test-Path $subPackagesPath) {
...
$packagesNextjsPath = Find-PackagesNextjs
...
docker exec $NAME bash -lc "command -v curl >/dev/null 2>&1 && curl -sf http://localhost:3001 >/dev/null 2>&1" | Out-Null
